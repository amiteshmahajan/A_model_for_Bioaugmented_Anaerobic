...
public class PovRayWriter implements Serializable {
	...
	public void initPovRay(Simulator aSim, String outPath) {
		...
		//	incFile.copyTo(new File(dir+"povray.zip"+File.separator+"sceneheader.inc"));     qanita

			incFile=new File(dir+"lastIter"+File.separator+"scenefooter.inc");
	//		incFile.copyTo(new File(dir+"povray.zip"+File.separator+"scenefooter.inc"));   qanita
	...
	public void write(int fileIndex) {
		try {
			...
			//System.out.println("Creating the povray file");
			File f = new File(_povRay.writeModelState(dir+"lastIter"+File.separator+"it(last).pov"));

			// Copy the povray file inside the archive
		//	f.copyTo(new File(dir+"povray.zip"+File.separator+"it("+LogFile.numberToString(fileIndex)+").pov"));   qanita
			f.copyTo(new File(dir+"povray.zip"+File.separator+"it("+LogFile.numberToString(fileIndex)+").pov"));
			File.update(_vArchive);
	...
	//Revision 2. Farzin
	public void writeTuring(int fileIndex) {
		try {
			// Create the povray file
			File f = new File(_povRay.writeModelStateForTuring(dir+"lastIter"+File.separator+"tur_it(last).pov"));

			// Copy the povray file inside the archive
		//	f.copyTo(new File(dir+"povray.zip"+File.separator+"tur_it("+LogFile.numberToString(fileIndex)+").pov"));   qanita
			File.update(_vArchive);
