...
public class Biofilm3D implements Serializable {
	...
	private void initializeAll() {
		...
		for (AllBC aBdry : _pov.mySim.world.domainList.get(0).getAllBoundaries()) {
			if (aBdry.isSupport()
					&& !aBdry.getSide().equals("x0y") && !aBdry.getSide().equals("yNz")) {//It was yNz. Changed by Farzin for 2D
				//xNy for 2D. yNz for 3D //
	...
	protected void particlesToFile(FileWriter f) throws IOException {
		for (Agent anAgent : _pov.mySim.agentGrid.agentList) {
			ParticleWithCapsule thisParticle = new ParticleWithCapsule((LocatedAgent)anAgent);
			// TJ Flann
			Iterator<LocatedAgent> iter = ((LocatedAgent)anAgent).getMyTightJunctions().iterator();
			while (iter.hasNext()) {
				LocatedAgent aNeighbor = iter.next();
				ParticleWithCapsule thisNeighborParticle = new ParticleWithCapsule((LocatedAgent)aNeighbor);
				if(Math.abs(aNeighbor._location.x-((LocatedAgent)anAgent)._location.x)<150
						&&Math.abs(aNeighbor._location.y-((LocatedAgent)anAgent)._location.y)<150
						&& Math.abs(aNeighbor._location.z-((LocatedAgent)anAgent)._location.z)<150)
					f.write(thisParticle.toTightJunctionString(thisNeighborParticle));
			}
		} // write all the particles after the tight junctions
		for (Agent anAgent : _pov.mySim.agentGrid.agentList) {
			ParticleWithCapsule thisParticle = new ParticleWithCapsule((LocatedAgent)anAgent);
			f.write(thisParticle.toString());
		}
	}

	protected void turingParticlesToFile(FileWriter f) throws IOException {
		for (Agent anAgent : _pov.mySim.agentGrid.agentList) {
			//Yeast
			ParticleWithCapsule s = new ParticleWithCapsule((Yeast)anAgent);
			f.write(s.toString());
		}
	}
}
