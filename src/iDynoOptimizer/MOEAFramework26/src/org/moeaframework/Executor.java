...
import iDynoOptimizer.MOEAFramework26.src.org.moeaframework.algorithm.Checkpoints;
...
import iDynoOptimizer.MOEAFramework26.src.org.moeaframework.util.distributed.DistributedProblem;
import iDynoOptimizer.MOEAFramework26.src.org.moeaframework.util.io.FileUtils;
import iDynoOptimizer.MOEAFramework26.src.org.moeaframework.util.progress.ProgressHelper;
import iDynoOptimizer.MOEAFramework26.src.org.moeaframework.util.progress.ProgressListener;
...
public class Executor extends ProblemBuilder {
    ...
    public Executor withMaxGenerations(int maxGenerations) {
        withProperty("maxGenerations", maxGenerations);
     ...
     * @return a reference to this executor
     */
    public Executor withProperties(Properties properties, boolean clearExistingProperties) {
        if(clearExistingProperties)this.properties.clear();
    ...
    public List<NondominatedPopulation> runSeeds(int numberOfSeeds) {
        ...
        int maxGenerations = properties.getInt("maxGenerations", 100);
    ...
    public NondominatedPopulation run() {
        isCanceled.set(false);
        return runSingleSeed(1, 1, properties.getInt("maxEvaluations", 25000), properties.getInt("maxGenerations", 100));
    }
    ...
    protected NondominatedPopulation runSingleSeed(int seed, int numberOfSeeds,
                                                   int maxEvaluations, int maxGenerations) {
            ...
            try {
                ...
                try {
                    ...
                    while (!algorithm.isTerminated() &&
                            (algorithm.getNumberOfEvaluations() < maxEvaluations) && (algorithm.getNumberOfGenerations() < maxGenerations)) {
