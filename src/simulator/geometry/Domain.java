...
public class Domain implements IsComputationDomain {
	...
	//sonia:chemostat 19.02.2010 changed visibility to public
	...
	protected SoluteGrid      _boundaryLayer, _diffusivityGrid, _biomassGrid, _yeastGrid;
	...
	protected boolean          hasBulkAsMedium;	
	...
	public Domain(Simulator aSim, XMLParser cdRoot) {
		...
		hasBulkAsMedium=cdRoot.getParamBool("hasBulk");
		...
		_yeastGrid = createGrid( "totalYeast", 0);
		...
		_diffusivityGrid = createGrid( "diffusivityGrid", 0);
	...
	public void refreshBioFilmGrids() {
		...
		currentSim.agentGrid.fitAgentMassOnGrid(_biomassGrid);
		//Added by Farzin 
		currentSim.agentGrid.fitAgentMassOnGrid(_yeastGrid,"Yeast");
			...
			for (int j = 1; j<_nJ+1; j++) {
				for (int k = 1; k<_nK+1; k++) {
					if (_biomassGrid.grid[i][j][k]>0) {
						...
						//Code changed to original form by Amitesh
	...
	public SoluteGrid getYeastGrid() {
		return _yeastGrid;
	}
