...
public class LocatedParam extends ActiveParam {
	...
	//created to by honey to create a random agitation in the system
	public double agitationCV  = 0.0;


	// Minimal radius before death (in um)
	...
	//Added by Farzin; to have fixed particles. These particles only move when the applied force is so high*
	public boolean fixed=false;
	
	//Added by Farzin for Yeast model
	public double springFactor		= 0;
	public double springLimit		= 0;
	...
	public void init(Simulator aSim, XMLParser aSpeciesRoot) {
		...
		value = aSpeciesRoot.getParamDbl("agitationCV");
		if(!Double.isNaN(value)) agitationCV = value;
		...
		//Added by Farzin to cancel the movement for fixed particles
		fixed = aSpeciesRoot.getParamBool("fixed");
		//Added by Farzin to read spring movement parameters for Yeast model
//		value = aSpeciesRoot.getParamLength("springFactor");
//		if(!Double.isNaN(value)) springFactor = value;
//		
//		value = aSpeciesRoot.getParamLength("springLimit");
//		if(!Double.isNaN(value)) springLimit = value;
//		
	}
