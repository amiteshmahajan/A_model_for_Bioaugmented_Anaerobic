...
public abstract class LevelSet {
	...
	public void refreshBorder(boolean evalErosion, Simulator aSim) {
		...
		//System.out.println(" current timestep value: "+ timeStep );
			...
			if (aGroup.nFreeNbh==0) {
				...
				/*if (evalErosion) {
					...
					if (dSpeed != 0.0)
						...
						aGroup.erosionTime = 0;  // changed to 0 by Amitesh  to ensure slaughting takes place
					else
						aGroup.erosionTime = INF;
				}*/
	...
	public void computeLevelSet(Simulator aSim) {
		//System.out.println("computeLevelSet method called");
		LocatedGroup trial;
		int index;

		while (_close.size()>0) {
			System.out.println("inside compute level set method, close.size>0");
	...
	/*protected Double getLocalDetachmentSpeed(LocatedGroup aGroup, Simulator aSim) {
		System.out.println("called getDetachment method inside LevelSet class");
		System.out.println("aGroup.cc.x = " + aGroup.cc.x +"aGroup.cc.y = " + aGroup.cc.y );
		double distance = Math.sqrt((225-aGroup.cc.x)*(225-aGroup.cc.x) + (225-aGroup.cc.y)*(225-aGroup.cc.y));
	...
	private double computeTValue(LocatedGroup aGroup, Simulator aSim) {
		...
		//System.out.println("Inside computeTValue function: ");
